...
class SelfBehavior(object):
    ...
    def load_data(self, self_path, desc, pubr):
        self_li = []
        index = 0
        ...
        with open(self_path) as f:
            ...
            while line:
                line = line.replace('\n', '')
                att_li = line.split('# #')
                embed = self.content_embed(content=att_li[-1])
                self_li.append([float(att_li[0]), embed])
                line = f.readline()
                index += 1
                pubr.desc = f'{desc}-->{index}'
                pubr.update(1 / task_len)
    ...
    def wv_self_behaviour(self, user_path, desc, pubr):
        self_path = f'{user_path}/self.txt'
        self_li = self.load_data(self_path, desc=desc, pubr=pubr)
        wv_li = []
