...
class User2AudioDataset(object):
    ...
    def get_item(self, index_li, desc, pubr):
        index_li = index_li.numpy()
        self_matrix, participant_matrix, topic_label = [], [], []
        for index in index_li:
            self_behaviour, participant_behaviour, label = self.iteration(index=index, desc=desc, pubr=pubr)
    ...
    def iteration(self, index, desc, pubr):
        index = str(index)
        user_path = self.path_ref + '/' + self.data[index]['path']
        self_behaviour = self.sb.wv_self_behaviour(user_path=user_path, desc=desc, pubr=pubr)
